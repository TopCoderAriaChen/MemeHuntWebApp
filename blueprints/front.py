...
def mycomment():
  ...
  if 'user_id' in session.keys():
    ...
    query_obj=(db.session.query(PostModel).join(CommentModel,CommentModel.post_id==PostModel.id)
               .filter(PostModel.is_active==True)
               .filter(CommentModel.author_id==context['current_user']))
...
def myrank():
    ...
    if 'user_id' in session:
        ...
        query_obj = execute_sql("SELECT username, number_of_post, number_of_comment, credits, level, id FROM view_comment_rank")
